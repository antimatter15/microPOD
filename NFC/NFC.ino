// This code runs on ESP32-S3 with a PN532 board configured for UART (default 0, 0 switches)
// where TXD is connected to GPIO13, and RXD is connected to GPIO12
// ArduinoJson is the only dependency. 
// Libraries for PN532 and NDEF have been vendored into mini_nfc.h
...
//When using Seeed PN532 and NFC libraries uncomment out the below lines
...
uint8_t ndefBuf[1600];
NdefMessage ndmessage;
int messageSize;
...
void ecdsa_test() {
  ...
  // Make sure you create these keys in alphabetical order or else 
  ...
  doc["zupass_description"]["value"] = "#" + String(number) + " Generated by ESP32-S3";
  ...
  doc["zupass_title"]["value"] = "ESP32 NFC POD #" + String(number);
  ...
  mbedtls_mpi_free(&ss); mbedtls_mpi_free(&s); mbedtls_mpi_free(&S); mbedtls_mpi_free(&r);


  StaticJsonDocument<1400> pcd;
  ...
  StaticJsonDocument<1400> addLink;
  addLink["type"] = "Add";
  ...
  ndmessage = NdefMessage();
  ndmessage.addUriRecord(zupassURL);
  messageSize = ndmessage.getEncodedSize();
